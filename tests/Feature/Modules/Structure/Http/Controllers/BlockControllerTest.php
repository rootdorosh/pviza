...
class BlockControllerTest extends TestCase
{
    ...
    public function index()
    {
        ...
        $response = $this->json('GET', $url, [], self::$headers);
        $response->assertStatus(200);        
        $this->saveResponse($response, 'structure/blocks/index', 200);        
   }
    ...
    public function show()
    {
        ...
        $url = self::BASE_URL . 'structure/domains/' . $this->domain->id . '/pages/' . $this->page->id . '/blocks/' . $block->alias;
        $response = $this->json('GET', $url, [], self::$headers);
        $response->assertStatus(200);        
        $this->saveResponse($response, 'structure/blocks/show', 200);        

        $url = self::BASE_URL . 'structure/domains/' . $this->domain->id . '/pages/' . $this->page->id . '/blocks/content20';
        $response = $this->json('GET', $url, [], self::$headers);
        $response->assertStatus(204);        
        $this->saveResponse($response, 'structure/blocks/show', 204);        
    }
