...
class Controller extends Base
{
    ...
    public function getMeta(): string
    {
        ...
        if (!empty($this->model['parentModel'])) {
            $parentModelType = !empty($this->model['parentModelConf']['widget']) ? $this->model['parentModelConf']['widget'] : 'listgrid';
            ...
            if ($parentModelType === 'listgrid') {
                ...
                foreach ($this->model['fields'] as $attr => $field) {
                    ...
                    if ($field['uiType'] === 'select' && !empty($field['options'])) {
                ...
                foreach ($this->model['fields'] as $attr => $field) {
                    ...
                    if (!empty($field['relation']) && $field['relation']['type'] === 'BelongsTo') {
                        ...
                        $attrName = Str::before($attr, '_id') . '_' . $relationTitleAttr;
                    }    
                    
                    if (!empty($field['uiType']) && $field['uiType'] === 'image') {
                        $renderable = 'image';
                    }

                    $raw.= "\t\t\t\t\t" . "'{$attrName}' => [\n";
                    $raw.= "\t\t\t\t\t\t'renderable' => '".$renderable."',\n";
                    $raw.= "\t\t\t\t\t],\n";
                }
                $raw.= "\t\t\t\t],\n";
            }    
