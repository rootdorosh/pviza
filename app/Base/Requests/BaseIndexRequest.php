...
abstract class BaseIndexRequest extends BaseRequest
{
     ...
     * @return array
     */
    public function sortParams(): array
    {
        $sortDir = $this->attr('sort_dir');
        $sortAttr = $this->attr('sort_attr');
        $sortAttrMap = static::sortAttrMap();
        if ($sortAttr && isset($sortAttrMap[$sortAttr])) {
            $sortAttr = $sortAttrMap[$sortAttr];
        }
        
        return [$sortAttr, $sortDir];
    }
    ...
    public function paginate()
    {
        ...
        list($sortAttr, $sortDir) = $this->sortParams();
        if ($sortDir && $sortAttr) {
